...
function addWarga($data) {
	...
	$query = mysqli_query($conn, "INSERT INTO warga VALUES ('', '$nik', '$nama_warga', '$jenis_kelamin', '$no_telepon', '$email', '$alamat', '$status')");
  	return mysqli_affected_rows($conn);
}
...
function deleteWarga($id) {
	global $conn;
	if (checkJabatan() == true) {
		$query = mysqli_query($conn, "DELETE FROM warga WHERE id_warga = '$id'");
	  	return mysqli_affected_rows($conn);
	}
}
...
function editWarga($data) {
	...
	$alamat = htmlspecialchars($data['alamat']);
	$status = htmlspecialchars($data['status']);
	$query = mysqli_query($conn, "UPDATE warga SET nama_warga = '$nama_warga', jenis_kelamin = '$jenis_kelamin', no_telepon = '$no_telepon', email = '$email' WHERE id_warga = '$id_warga'");
  	return mysqli_affected_rows($conn);
}
...
function addBulanPembayaran($data) {
	...
	if (mysqli_fetch_assoc($check_bulan)) {
		...
		$dataWarga = mysqli_query($conn, "SELECT * FROM warga");
		$warga = "INSERT INTO uang_kas (id_warga, id_bulan_pembayaran, minggu_ke_1, minggu_ke_2, minggu_ke_3, minggu_ke_4) VALUES ";
		foreach ($dataWarga as $dw) {
		 	$warga .= "('{$dw['id_warga']}','{$id_bulan_pembayaran}','','','','')";
            $warga .= ",";
		}
		$warga = rtrim($warga,",");
		$query_warga = mysqli_query($conn, $warga);
	  	return mysqli_affected_rows($conn);
	}
}
...
function tambahWargaUangKas($data) {
	global $conn;
	$id_warga = htmlspecialchars($data['id_warga']);
	$id_bulan_pembayaran = htmlspecialchars($data['id_bulan_pembayaran']);
	$query = mysqli_query($conn, "INSERT INTO uang_kas VALUES ('', '$id_warga', '$id_bulan_pembayaran', '0', '0', '0', '0')");
  	return mysqli_affected_rows($conn);
}
